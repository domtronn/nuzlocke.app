...
import { z } from 'zod'
import { IFights, IGames, ISaves, ITeams } from './types'
...
const bq = new BigQuery({
  ...
  credentials: {
    ...
    client_x509_cert_url:
      'https://www.googleapis.com/robot/v1/metadata/x509/nuzlocke-data-syncer%40nuzlocke-tracker-data.iam.gserviceaccount.com'
  }
})

export const processGame = async ({ data, user_id }: IGames) => {
  try {
    const rows = data.map(({ created_at, updated_at, ...d }) => ({
      ...d,
      user_id,
