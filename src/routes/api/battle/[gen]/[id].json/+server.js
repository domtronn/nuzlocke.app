  ...
  return map(
    compose(
      (d) => {
        ...
        return nonnull({
          ...d,
          name: patched ? titleCase(patched?.locale || d.name) : d.name,
          type: patched?.type || d.type,
          power: patched?.power || d.power,
          effect: patched?.effect || d.effect,
          damage_class:
            damageClass != 'status' && physicalSpecialSplit
...
const toHeld = (held, patch) => {
  ...
  return nonnull({
    ...
    effect:
      held.effect_entries[0]?.short_effect ||
        ...
        .map(async (p) => {
          try {
            ...
            const abilities = await Promise.all(
              p.abilities.map((a) =>
                maybe(P.getAbilityByName, a).catch((_) => p.ability)
              )
            )
            ...
            return nonnull({
              ...
              abilities:
                abilities.length > 1
                  ? abilities.map((a) => toAbility(a, patch.ability))
                  : null
            })
          } catch (e) {
            console.error(`[${url}] Error fetching ${p.name}`)
            // Verify pokemon fetch works
            await P.getPokemonByName(p.name).catch((_) =>
              console.error('E_GET_POKEMON: ' + p.name)
            )
