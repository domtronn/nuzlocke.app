...
export default (box, boss) => {
  ...
  const calcAdvMap = (compf, mods) => mods
     .reduce((acc, [type, mod]) => {
  ...
  const calcModPct = (compf, mods, types, debug) => mods.reduce((acc, type) => {
  ...
  const bossTeamResist = calcAdvMap(mod => mod < 2 && mod !== 0, bossTeamDmgMods)
  const bossTeamImmunity = calcAdvMap(mod => mod === 0, bossTeamDmgMods)

  const recommendedMons = box.map(({ alias: defName, baseStats: stats, types: defTypes }) => {
    ...
    */
    ...
    const defAdvantageScore = bossMoveTypes.reduce((acc, type) => {
      ...
      switch (damageMod) {
      case 0: return acc + 8
      case 0.25: return acc + 4
      case 0.5: return acc + 2
      case 2: return acc - 4
      case 4: return acc - 2
      default: return acc
      }
    ...
    if (debug.includes(defName)) console.log(defName, '---', offAdvantageScore, defAdvantageScore)
    ...
    */
    ...
    return {
      ...
      weakPct: calcModPct(m => m > 1, bossMoveTypes, defTypes, debug.includes(defName)),
      resistPct: calcModPct(m => m < 1, bossMoveTypes, defTypes, debug.includes(defName)),
      immunePct: calcModPct(m => m === 0, bossMoveTypes, defTypes, debug.includes(defName)),
    }
  })
    .filter(a => a.offAdv > 0 && a.defAdv > 0 && a.weakPct < 0.5 && a.resistPct > 0)
    .sort((a, b) => {
    return b.offAdv - a.offAdv
        || b.defAdv - a.defAdv
        || b.immunePct - a.immunePct
        || b.resistPct - a.resistPct
        || a.weakPct - b.weakPct
    })
    .map(p => ({ name: p.name, weakPct: p.weakPct, immunePct: p.immunePct, resistPct: p.resistPct }))

  return {
    recommendations: recommendedMons,
    // advice: [
    //   [...new Set(bossMoveClass)].length === 1
    //     ? `pure_${bossMoveClass[0]}`
    //     : '',
    //   boss
    // ].filter(i => i)
    // typeRecommendations: {
    //   total: boss.length,
    //   super: bossTeamWeak4x,
    //   weak: bossTeamWeak,
    //   resist: bossTeamResist,
    //   immune: bossTeamImmunity,
    // }
  }
}
