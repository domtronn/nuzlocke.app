...
const routeSearch = (s, route) => {
  ...
  return normalise(name).includes(s)
...
const pokemonSearch = (s, item) => {
  ...
  return normalise(pokemon).includes(s) // Search by pokemon name
    || normalise(nickname).includes(s)  // Search by nickname
    || normalise(NuzlockeStates[status]?.state).includes(s) // Search by status status
}
...
export const filterEntry = (filters, search, data, progress) => (route) => {
  ...
  const showUpcoming = fMain === 'upcoming' || (fMain === 'route' && fRoute === 'upcoming')
