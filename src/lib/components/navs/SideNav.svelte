...
<Portal target='#sidenav_button' hidden>
  <button
    ...
    class='h-full transition dark:text-gray-500 dark:hover:text-gray-200'
    >
...
{#if show}
  ...
  >
    <button on:click={_ => show = !show}>
      <Icon src={X} size='1.8rem' className='-ml-2 mb-2 fill-current transition-colors hover:cursor-pointer dark:text-gray-500 dark:hover:text-gray-200' />
    </button>

    {#each groups as group}
      <h3 class='text-lg font-light dark:text-gray-200 mt-4'>{capitalise(unslugify(group))}</h3>
      {#each grouped[group] as b}
        <li class='text-xs underline hover:text-gray-200 hover:scale-110 hover:cursor-pointer origin-left transition' on:click={onnav(b.oid)}>
          {b.name}
        </li>
      {/each}
    {/each}
    ...
    <li
      ...
      class='text-sm inline-flex items-center gap-x-1 mt-4 underline transition hover:text-gray-200 hover:cursor-pointer'
      >
